...
class DenseCapModel(GeneralizedRCNN):
    ...
    def __init__(self, backbone, return_features = False,
                 # Caption parameters
                 box_describer=None,
                 feat_size=None, hidden_size=None, max_len=None,#4096,512,16
                 emb_size=None, rnn_num_layers=None, vocab_size=None,#512,1,10629
        ...
        if box_describer is None:
            for param in {max_len, emb_size, rnn_num_layers, vocab_size}:#設定するパラメータが整数であるかつ、0以上であることの確認
        ...
        if rpn_anchor_generator is None:#アンカーボックスを定義
        ...
        rpn = RegionProposalNetwork(
            ...
            rpn_fg_iou_thresh, rpn_bg_iou_thresh,#rpn_fg_iou_thresh=0.7, rpn_bg_iou_thresh=0.3
            rpn_batch_size_per_image, rpn_positive_fraction,#rpn_batch_size_per_image=256, rpn_positive_fraction=0.5
            rpn_pre_nms_top_n, rpn_post_nms_top_n, rpn_nms_thresh)# rpn_nms_thresh=0.7
        ...
        if box_head is None:
            ...
            representation_size = 4096 if feat_size is None else feat_size#feat_sizeは4096
            box_head = TwoMLPHead(
...
class TwoMLPHead(nn.Module):
    ...
    def __init__(self, in_channels, representation_size):#in_channels = out_channels * resolution ** 2,  representation_size = 4096
