...
class BoxDescriber(nn.Module):
    ...
    def __init__(self, feat_size, hidden_size, max_len, emb_size, rnn_num_layers, vocab_size,
        ...
        self.rnn_num_layers = rnn_num_layers#1
    ...
    def init_hidden(self, batch_size, device):
        ...
        h0 = torch.zeros(self.rnn_num_layers, batch_size, self.hidden_size).to(device)# self.rnn_num_layers = 1 self.hidden_size = 512
    ...
    def forward_train(self, feat, cap_gt, cap_lens):
        ...
        入力
            feat : (batch_size(検出数によって様々) , 4096) 例：torch.Size([183, 4096])
        
        出力
            predicts : (batch_size(検出数によって様々), 文字列長, 語彙数) 例：torch.Size([183, 50, 10629])
        ...
        #print(feat.shape) #(検出数によって様々 , 4096) 例：torch.Size([183, 4096])

        word_emb = self.embedding_layer(cap_gt)  # (batch_size, max_len, embed_size) 
        feat_emb = self.feature_project_layer(feat)  # (batch_size, embed_size) (検出数によって様々 , 512)
        #print(feat_emb.shape)

        h0, c0 = self.init_hidden(batch_size, device)
        if self.fusion_type == 'init_inject':#fusion_type='init_inject'
            _, (h0, c0) = self.rnn(feat_emb.unsqueeze(1), (h0, c0))  # first input projected feat emb to rnn

        rnn_input_pps = pack_padded_sequence(word_emb, lengths=cap_lens, batch_first=True, enforce_sorted=False)#0埋めして返される
        ...
        #print(rnn_output.shape)# (batch_size, 文字列長(16とか), hiden_size(512)) 例：torch.Size([183, 50, 512])
        predicts = self.fc_layer(rnn_output)
        #print(predicts.shape)# (batch_size, 文字列長, 語彙数) 例：torch.Size([183, 50, 10629])
