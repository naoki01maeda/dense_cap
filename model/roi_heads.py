...
class DenseCapRoIHeads(nn.Module):
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        #print(features["3"].shape)#featuresはdict
        box_features = self.box_roi_pool(features, proposals, image_shapes)
        #print(box_features.shape)#torch.Size([4096, 256, 7, 7])
        box_features = self.box_head(box_features)
        #print(box_features.shape)#torch.Size([4096, 4096])
        logits, box_regression = self.box_predictor(box_features)#損失を計算するための処理
        
        if self.training:
            # labels 到这里应该是有0和（1，class-1），0代表背景，其余代表类别，需要剔除背景，然后进行描述(List[Tensor])
            # 也需要滤除对应的caption和caption_length
            keep_ids = [label>0 for label in labels]#背景を表す0の要素はfalse
            boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]
        ...
        #print(box_features.shape)# box_features 例：torch.Size([183, 4096]) 背景は省かれる

        caption_predicts = self.box_describer(box_features, caption_gt, caption_length)#文章予測
