...
IMG_DIR_ROOT = './data/ori_data'#'./data/visual-genome'
VG_DATA_PATH = './data/ori_VG-regions-lite.h5'#'./data/VG-regions-lite.h5'
LOOK_UP_TABLES_PATH = './data/ori_VG-regions-dicts-lite.pkl'#'./data/VG-regions-dicts-lite.pkl'
...
def save_model(model, optimizer, amp_, results_on_val, iter_counter, flag=None):#モデル設定の保存
...
def train(args):
    ...
    print('Model {} start training...'.format(MODEL_NAME))#モデルを読み込んで、訓練をスタートする

    #モデル定義
    model = densecap_resnet50_fpn(backbone_pretrained=args['backbone_pretrained'],#True
                                  feat_size=args['feat_size'],#4096
                                  hidden_size=args['hidden_size'],#512
                                  max_len=args['max_len'],#16
                                  emb_size=args['emb_size'],#512
                                  rnn_num_layers=args['rnn_num_layers'],#1
                                  vocab_size=args['vocab_size'],#10629
                                  fusion_type=args['fusion_type'],#'init_inject'
                                  box_detections_per_img=args['box_detections_per_img'])#50
    if args['use_pretrain_fasterrcnn']:#True
    ...
    train_set = DenseCapDataset(IMG_DIR_ROOT, VG_DATA_PATH, LOOK_UP_TABLES_PATH, dataset_type='train')#IMG_DIR_ROOT = './data/visual-genome' VG_DATA_PATH = './data/VG-regions-lite.h5' LOOK_UP_TABLES_PATH = './data/VG-regions-dicts-lite.pkl'
    ...
    if MAX_TRAIN_IMAGE > 0:
        train_set = Subset(train_set, range(MAX_TRAIN_IMAGE))#引数にデータ数を渡して、絞る
    if MAX_VAL_IMAGE > 0:
        val_set = Subset(val_set, range(MAX_VAL_IMAGE))#引数にデータ数を渡して、絞る
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for batch, (img, targets, info) in enumerate(train_loader):
            ...
            #bbox損失
            #losses['loss_classifier'] + losses['loss_box_reg']はroi_head.pyで定義
            detect_loss =  losses['loss_objectness'] + losses['loss_rpn_box_reg'] + \
                           losses['loss_classifier'] + losses['loss_box_reg']#losses['loss_objectness'] + losses['loss_rpn_box_reg']はtorchvision.models.detection.rpn import RegionProposalNetwork　で定義されている
            #キャプション損失
            caption_loss = losses['loss_caption']#roi_head.pyで定義
            ...
            if iter_counter > 0 and iter_counter % 1000 == 0:
                try:
                    ...
                    #save_model(model, optimizer, amp, None, iter_counter)
                    pass
                    #if USE_TB:
                        #pass
    ...
    #save_model(model, optimizer, amp, None, iter_counter, flag='end')#results
